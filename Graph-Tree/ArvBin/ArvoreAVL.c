...
int consulta_ArvAVL(ArvAVL *raiz, char *valor, student *st){
    if(raiz == NULL)
        return 0;
    struct NO* atual = *raiz;
    while(atual != NULL){
        if(strcmp(valor, atual->info.name) == 0){ // achei
            *st = atual->info;
            return 1;
        }
        if(strcmp(valor, atual->info.name) > 0) // para dir
            atual = atual->dir;
        else
            atual = atual->esq;                 // para esq
    }
    return 0;                                   // n achou
}
...
int insere_ArvAVL(ArvAVL *raiz, student valor){
    int res;
    if(*raiz == NULL){//árvore vazia ou nó folha
        ...
        *raiz = novo; // *raiz == ponteiro para a raiz
        return 1;
    }

    struct NO *atual = *raiz;
    if(strcmp(valor.name, atual->info.name) < 0){
        if((res = insere_ArvAVL(&(atual->esq), valor)) == 1){
            if(fatorBalanceamento_NO(atual) >= 2){
                if(strcmp(valor.name, (*raiz)->esq->info.name) < 0){
                    RotacaoLL(raiz);
                }else{
                    RotacaoLR(raiz);
                }
        ...
        if(strcmp(valor.name, atual->info.name) > 0){
            if((res = insere_ArvAVL(&(atual->dir), valor)) == 1){
                if(fatorBalanceamento_NO(atual) >= 2){
                    if(strcmp(valor.name, (*raiz)->dir->info.name) > 0){
                        RotacaoRR(raiz);
                    }else{
                        RotacaoRL(raiz);
                    }
